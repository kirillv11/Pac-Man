...
class Pacman:
    ...
    def init_texture(self):
        image_pacman_right = pyray.load_image(
            './resources/image/pacman_right.png')
        image_pacman_left = pyray.load_image(
            './resources/image/pacman_left.png')
        image_pacman_bottom = pyray.load_image(
            './resources/image/pacman_bottom.png')
        image_pacman_forward = pyray.load_image(
            './resources/image/pacman_forward.png')
        ...
        self.texture_bottom = pyray.load_texture_from_image(
            image_pacman_bottom)
        self.texture_forward = pyray.load_texture_from_image(
            image_pacman_forward)
    ...
    def logic(self, map, cell_size, cels, ghosts, life_counter, game_scene_instance):
        ...
        if self.x % cell_size == 0 and self.y % cell_size == 0:
            ...
            if 2 in walls:
                ...
                if self.direction not in walls_named:
                    self.direction = "none"
            self.seed_collision(self.x // cell_size, self.y // cell_size, cels)

        self.step(self.direction, self.speed)

    # проверка на столкновение с призраками (на вход идёт массив призраков)
    def collides_ghost(self, ghosts):
        col = False
        x = self.x + 20
        y = self.y + 20
        for ghost in ghosts:
